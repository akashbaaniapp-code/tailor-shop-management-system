...
interface Tailor { id: string; name: string; phone?: string; address?: string }

export default function SetupTailor() {
  const [items, setItems] = useState<Tailor[]>([])
  const [loading, setLoading] = useState(true)
  const [showForm, setShowForm] = useState(false)
  const [editItem, setEditItem] = useState<Tailor | null>(null)
  ...
  async function load() {
    setLoading(true)
    try {
      const res = await api.listTailors()
      setItems(res.items)
    } catch (err: any) {
  ...
  async function handleSave() {
    ...
    try {
      if (editItem) {
        await api.updateTailor({ id: editItem.id, name: name.trim(), phone, address })
        toast.success('Tailor updated')
      } else {
        await api.createTailor({ name: name.trim(), phone, address })
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await api.deleteTailor(id); toast.success('Deleted'); load()
    } catch (err: any) {
      toast.error(err.message)
    }
  }
      ...
      >
        <div>
          <p style={{ fontSize: 14, color: '#e8eae9', lineHeight: 1.6, margin: 0 }}>
            Manage tailors working in your shop
            <span style={{ display: 'block', marginTop: 2, color: '#555', fontSize: 12 }}>
              Assign tailors to orders and track their work
            </span>
          </p>
        </div>
        <button
      ...
      <div style={{ background: '#14161a', border: '1px solid #2a2d33', borderRadius: 16, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '48px 0' }}>
            ...
            <p style={{ color: '#888' }}>No tailors yet</p>
            <p style={{ fontSize: 12, color: '#888', marginTop: 4 }}>Add your first tailor to start assigning work</p>
          </div>
        ) : (
      ...
      {showForm && (
        <Dialog open onOpenChange={setShowForm}>
          <DialogContent style={{ background: '#1a1c1e', border: '1px solid #2a2d33', maxWidth: 400 }}>
            <DialogHeader>
              <DialogTitle style={{ color: '#fff' }}>{editItem ? 'Edit Tailor' : 'Add Tailor'}</DialogTitle>
            </DialogHeader>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div>
                <label style={{ color: '#888', fontSize: 13, display: 'block', marginBottom: 6 }}>Phone</label>
                <input
                  value={phone}
