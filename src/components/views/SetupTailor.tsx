      ...
      <Card className="border-slate-200">
        <CardContent className="p-4">
          <div className="flex justify-between items-center">
            <p className="text-sm text-slate-600">Manage tailors working in your shop</p>
            <Button onClick={openCreate} className="bg-emerald-600 hover:bg-emerald-700">
              <Plus className="w-4 h-4 mr-1" /> Add Tailor
            </Button>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-500">No tailors yet</p>
            </div>
          ) : (
                ...
                <tbody>
                  {items.map(it => (
                    <tr key={it.id} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="px-4 py-2.5 text-slate-600">{it.phone || '-'}</td>
      ...
      {showForm && (
        <Dialog open onOpenChange={setShowForm}>
          <DialogContent className="max-w-md">
            <DialogHeader><DialogTitle>{editItem ? 'Edit Tailor' : 'Add Tailor'}</DialogTitle></DialogHeader>
